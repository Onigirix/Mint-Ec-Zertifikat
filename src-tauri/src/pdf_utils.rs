...
pub fn encode_pdf_string(text: &str) -> Vec<u8> {
    ...
    encoded.push(0xFE);
    encoded.push(0xFF);
    encoded.extend(text.encode_utf16().flat_map(|unit| unit.to_be_bytes()));
    encoded
}

pub fn string_from_pdf_object(object: &Object) -> Option<String> {
    if let Object::String(ref data, _) = object {
        Some(decode_pdf_string(data))
    } else {
        None
    }
}

pub fn name_to_string(object: &Object) -> Option<String> {
    if let Object::Name(ref data) = object {
        from_utf8(data).ok().map(|value| value.to_owned())
    } else {
        None
    }
}
